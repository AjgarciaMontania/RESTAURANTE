...
/**
 * Encoge y comprime la foto hasta que quepa, bajando primero la calidad y
 * después el tamaño. Devuelve un data URL listo para guardar.
 */
...
export async function subirFoto(file) {
  ...
  const id = uid() + uid();
  await setDoc(doc(db, "fotos", id), { datos, bytes: datos.length });
  cache.set(id, datos);
  return id;
}
